...
func (h *WorkspaceHandler) GetRunnerCredentials(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		errs.WriteJSON(w, http.StatusOK, map[string]any{
			...
			// Whether the instance provides a fallback GitHub token via env.
...
// DeleteRunnerCredentials disconnects one credential (?field=claude|github)
// or both (?field=all, the default).
