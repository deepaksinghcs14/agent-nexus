...
export default function RunsPage() {
  ...
  return (
    <div className="p-6">
      ...
      {!isLoading && !error && runs.length === 0 && <div className="border border-dashed border-gray-200 rounded-xl py-12 text-center"><Activity className="mx-auto text-gray-300 mb-3" /><p className="text-sm text-gray-500">No runs match these filters.</p></div>}

      {runs.length > 0 && <div className="bg-white border border-gray-100 rounded-xl overflow-x-auto">
        <table className="w-full text-[12px]">
          <thead><tr className="bg-gray-50 border-b border-gray-100">{['Run ID', 'Agent', 'Status', 'Tokens', 'Cost', 'Started', ''].map((h) => <th key={h} className="text-left px-4 py-2 text-[10px] font-medium text-gray-400 uppercase">{h}</th>)}</tr></thead>
          <tbody>{runs.map((run) => <tr key={run.id} className="border-b last:border-b-0 border-gray-50">
            <td className="px-4 py-2.5 font-mono text-[11px] text-gray-500">{run.id.slice(0, 12)}</td>
            <td className="px-4 py-2.5 font-medium text-gray-900">{agentNames[run.agent_id] ?? run.agent_id.slice(0, 8)}</td>
            <td className="px-4 py-2.5"><span className={`px-2 py-0.5 rounded-full text-[10px] font-medium ${statusColor(run.status)}`}>{run.status}</span></td>
            ...
            <td className="px-4 py-2.5"><Link href={`/runs/${run.id}`} className="text-[11px] px-2.5 py-1 border border-gray-200 rounded-md text-gray-600 hover:bg-gray-50">Trace</Link></td>
          </tr>)}</tbody>
        </table>
      </div>}
    </div>
