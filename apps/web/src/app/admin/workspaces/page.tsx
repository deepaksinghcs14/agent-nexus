  ...
  return (
    <div className="p-4 sm:p-6 max-w-5xl">
      <div className="mb-5">
        ...
        <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">Manage all workspaces on this instance</p>
      </div>
      {error && (
      ...
      {!isLoading && !error && workspaces.length === 0 && (
        <div className="border border-dashed border-gray-200 dark:border-gray-700 rounded-xl py-12 text-center">
          <Building2 className="mx-auto text-gray-300 dark:text-gray-600 mb-3" />
            ...
            {workspaces.map((ws) => (
              <div key={ws.id} className="flex items-center gap-4 px-4 py-3 border-b last:border-b-0 border-gray-50 text-[12px] hover:bg-gray-50/50">
                ...
                <div className="flex-1 min-w-0">
                  <p className="font-medium text-gray-900 dark:text-gray-100 truncate">{ws.display_name}</p>
                  <p className="text-[10px] text-gray-400 dark:text-gray-500 font-mono">{ws.name}</p>
                </div>
                {/* Stats */}
                <div className="flex items-center gap-4 text-[11px] text-gray-500 dark:text-gray-400 flex-shrink-0">
                  ...
                  <span className="text-gray-400 dark:text-gray-500 hidden md:block">{relativeTime(ws.created_at)}</span>
                </div>
                {/* Delete */}
                <button
                  onClick={() => { setConfirmDelete(ws); setDeleteError('') }}
                  className="p-1.5 rounded hover:bg-red-50 text-gray-300 dark:text-gray-600 hover:text-red-500 transition-colors flex-shrink-0"
      ...
      {confirmDelete && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-gray-900 rounded-xl shadow-lg w-full max-w-md mx-4 p-6">
            <h3 className="text-base font-semibold text-gray-900 dark:text-gray-100 mb-1">Delete workspace?</h3>
