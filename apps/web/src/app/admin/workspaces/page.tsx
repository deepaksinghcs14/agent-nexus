        ...
        <div className="min-w-[600px] px-4 sm:px-0">
          <div className="bg-white border border-gray-100 rounded-xl overflow-hidden">
            {workspaces.map((workspace) => (
              <div key={workspace.id} className="grid grid-cols-12 px-4 py-3 border-b last:border-b-0 border-gray-50 text-[12px]">
                <div className="col-span-5">
                  <p className="font-medium text-gray-900">{workspace.display_name}</p>
                  <p className="text-[10px] text-gray-400">{workspace.name}</p>
                </div>
                <span className="col-span-4 text-gray-500 font-mono text-[10px]">{workspace.owner_id}</span>
                <span className="col-span-3 text-gray-400">{relativeTime(workspace.created_at)}</span>
              </div>
            ))}
          </div>
        </div>
