  ...
  return (
    <div className="p-4 sm:p-6">
      <div className="flex flex-wrap items-center gap-3 justify-between mb-4">
        <h1 className="text-xl font-semibold text-gray-900">Users ({users.length})</h1>
        <input
          value={search}
          ...
          <div className="bg-white border border-gray-100 rounded-xl overflow-hidden">
            {users.map((user) => (
              <div key={user.id} className="grid grid-cols-12 items-center px-4 py-3 border-b last:border-b-0 border-gray-50 text-[12px]">
                ...
                <span className="col-span-2 text-gray-500">{user.is_admin ? 'admin' : 'member'}</span>
                <span className="col-span-2">
                  <span className={`text-[10px] px-2 py-0.5 rounded-full ${statusColor(user.is_active ? 'active' : 'failed')}`}>
                    {user.is_active ? 'active' : 'disabled'}
                  </span>
                </span>
                <span className="col-span-2 text-[10px] text-gray-400">{relativeTime(user.updated_at)}</span>
                <button onClick={() => update.mutate(user)} className="col-span-1 text-[10px] text-purple-600">
