      ...
      {data && (
        <>
          ...
          <div className={`flex flex-wrap items-center gap-3 border rounded-xl px-4 py-3 mb-4 ${
            !data.runner_configured ? 'border-gray-200 bg-gray-50'
              : runnerOk ? (stub ? 'border-amber-200 bg-amber-50/60' : 'border-green-200 bg-green-50/50')
              : 'border-red-200 bg-red-50/60'
          }`}>
            <Server size={16} className={!data.runner_configured ? 'text-gray-400' : runnerOk ? (stub ? 'text-amber-600' : 'text-green-600') : 'text-red-500'} />
            <div>
          ...
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-2 mb-5">
            {statOrder.map((s) => (
              <div key={s} className="bg-gray-50 rounded-lg p-2.5 text-center">
                <p className="text-lg font-semibold text-gray-800">{stats[s] ?? 0}</p>
          ...
          <div className="bg-white border border-gray-100 rounded-xl overflow-hidden">
            <div className="flex items-center gap-2 px-4 py-3 border-b border-gray-100">
              ...
              <span className="text-[11px] text-gray-400">approval + session waits, all workspaces, oldest first</span>
            </div>
            {waiting.length === 0 ? (
                  ...
                  <thead>
                    <tr className="bg-gray-50 border-b border-gray-100">
                      {['Run', 'Workspace', 'Agent', 'Status', 'Durable', 'Waiting'].map((hd) => (
                        <th key={hd} className="text-left px-4 py-2 text-[10px] font-medium text-gray-400 uppercase">{hd}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                      ...
                      return (
                        <tr key={r.run_id} className="border-b border-gray-50">
                          <td className="px-4 py-2">
                            <Link href={`/runs/${r.run_id}`} className="font-mono text-purple-600 hover:underline">
                              {r.run_id.slice(0, 8)}…
                            </Link>
                          </td>
                          <td className="px-4 py-2 text-gray-700">{r.workspace}</td>
                          <td className="px-4 py-2 text-gray-700">{r.agent_name || '—'}</td>
                          <td className="px-4 py-2">
                            <span className={`text-[10px] px-2 py-0.5 rounded-full ${statusColor(r.status)}`}>{r.status}</span>
                          </td>
                          <td className="px-4 py-2 text-gray-500">{r.wait_type}</td>
                          <td className="px-4 py-2">
                            <span className={`inline-flex items-center gap-1 ${stale ? 'text-amber-700 font-medium' : 'text-gray-500'}`}>
