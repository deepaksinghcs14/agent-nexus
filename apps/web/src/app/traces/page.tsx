...
export default function TracesPage() {
  ...
  return <div className="p-6">
    ...
    {runs.length > 0 && <div className="bg-white border border-gray-100 rounded-xl overflow-hidden">{runs.map((run) => <Link key={run.id} href={`/traces/${run.id}`} className="grid grid-cols-12 items-center px-4 py-3 border-b last:border-b-0 border-gray-50 hover:bg-gray-50">
      <div className="col-span-3"><p className="font-mono text-[11px] text-gray-700">{run.id}</p><p className="text-[10px] text-gray-400 mt-0.5">{relativeTime(run.started_at)}</p></div>
      <span className="col-span-3 text-[12px] font-medium text-gray-800">{run.agent_id ? (names[run.agent_id] ?? run.agent_id.slice(0, 8)) : 'Group run'}</span>
      <span className="col-span-2"><span className={`text-[10px] px-2 py-0.5 rounded-full ${statusColor(run.status)}`}>{run.status}</span></span>
      <span className="col-span-2">{run.trigger_id ? <span className="inline-flex items-center gap-1 text-[10px] px-2 py-0.5 rounded-full bg-rose-50 text-rose-600 font-medium"><Zap className="w-2.5 h-2.5" />Webhook</span> : <span className="text-[10px] text-gray-400">Manual</span>}</span>
      <span className="col-span-1 text-[11px] text-gray-500">{formatTokens(run.total_input_tokens + run.total_output_tokens)}</span>
      <span className="col-span-1 text-[11px] text-purple-600 text-right">Inspect</span>
    </Link>)}</div>}
    <div className="mt-6 p-4 rounded-lg bg-gray-50 border border-gray-200">
      <p className="text-sm text-gray-600">
        Learn about SSE stream events and run step types in the{' '}
        <Link href="/docs/sse-events" className="text-[#534AB7] hover:underline">documentation</Link>.
      </p>
    </div>
  </div>
}
