    ...
    <div className="p-4 sm:p-6 max-w-3xl">
      <div className="flex flex-wrap items-center gap-3 justify-between mb-5">
        ...
        <div className="flex gap-2">
          <button onClick={() => router.push('/gateway/channels')} className="inline-flex items-center gap-1.5 px-3 py-1.5 border border-border-strong text-muted-foreground text-[12px] rounded-lg hover:bg-muted">
      ...
      <div className="space-y-5">
        ...
        <Field label="Agent">
          <select value={agentId} onChange={(e) => setAgentId(e.target.value)} className="w-full text-[13px] px-3 py-2 border border-border-strong rounded-lg bg-surface">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <Field label="Account ID" hint="Unique per WhatsApp number — different channels sharing the same number use the same ID">
                <input
                  ...
                  className="w-full text-[13px] px-3 py-2 border border-border-strong rounded-lg"
                />
              </Field>
              <Field label="Internal adapter URL"><input value={adapterURL} onChange={(e) => setAdapterURL(e.target.value)} className="w-full text-[13px] px-3 py-2 border border-border-strong rounded-lg" /></Field>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <Field label="DM policy"><select value={dmPolicy} onChange={(e) => setDMPolicy(e.target.value)} className="w-full text-[13px] px-3 py-2 border border-border-strong rounded-lg bg-surface"><option value="pairing">Pairing</option><option value="allowlist">Allowlist</option><option value="open">Open</option><option value="disabled">Disabled</option></select></Field>
              <Field label="Group policy"><select value={groupPolicy} onChange={(e) => setGroupPolicy(e.target.value)} className="w-full text-[13px] px-3 py-2 border border-border-strong rounded-lg bg-surface"><option value="disabled">Disabled</option><option value="allowlist">Allowlist</option><option value="open">Open</option></select></Field>
            </div>
          </>
        )}
        <div>
          <p className="text-[12px] font-medium text-foreground mb-2">Suggested skills</p>
          <div className="rounded-lg border border-border-strong divide-y divide-border">
            {skills.filter((s) => ['WhatsApp Formatter', 'Language Mirror', 'Safety Guardrail'].includes(s.name)).map((s) => (
              <label key={s.id} className="flex items-start gap-3 p-3 text-sm">
                ...
                <span><span className="font-medium text-foreground">{s.name}</span><span className="block text-xs text-muted-foreground">{s.description}</span></span>
              </label>
            ))}
          </div>
        </div>
