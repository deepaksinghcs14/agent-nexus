...
function CopyButton({ text }: { text: string }) {
  ...
  return (
    <button
      ...
      className="p-1 rounded hover:bg-gray-100 dark:hover:bg-gray-800 text-gray-400 dark:text-gray-500 hover:text-gray-600 dark:hover:text-gray-300"
  ...
  return (
    <div className="p-4 sm:p-6">
      <div className="flex flex-wrap items-center gap-3 justify-between mb-8">
        <div>
          <h1 className="text-xl font-semibold text-gray-900 dark:text-gray-100">Gateway Channels</h1>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Persistent channel entrypoints for always-on agents.</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="py-16 text-center">
          ...
          <p className="text-sm font-medium text-gray-600 dark:text-gray-400 mb-1">No gateway channels yet</p>
          <p className="text-sm text-gray-400 dark:text-gray-500 mb-4">Create a WhatsApp or HTTP channel to route messages into an agent.</p>
        ...
        <div className="rounded-lg border border-gray-200 dark:border-gray-700 overflow-hidden divide-y divide-gray-100 dark:divide-gray-800">
          {channels.map((c) => (
            <div key={c.id} className="bg-white dark:bg-gray-900 px-4 sm:px-5 py-4">
              <div className="flex flex-wrap items-start justify-between gap-4">
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className={`text-[10px] px-1.5 py-0.5 rounded-full font-medium ${c.is_active ? 'bg-emerald-50 dark:bg-emerald-500/10 text-emerald-700 dark:text-emerald-300' : 'bg-gray-100 dark:bg-gray-800 text-gray-500 dark:text-gray-400'}`}>
                      {c.is_active ? 'active' : 'disabled'}
                    </span>
                  </div>
                  {c.description && <p className="text-xs text-gray-400 dark:text-gray-500 mb-1">{c.description}</p>}
                  <p className="text-xs text-gray-500 dark:text-gray-400">Agent: {c.agent_name || c.agent_id}</p>
                  <div className="flex items-center gap-1 mt-2">
                    <code className="text-xs text-gray-500 dark:text-gray-400 font-mono bg-gray-50 dark:bg-gray-800/60 px-2 py-0.5 rounded truncate max-w-[200px] sm:max-w-md">{ingressURL(c)}</code>
                ...
                <div className="flex items-center gap-2 shrink-0">
                  <button onClick={() => toggle(c)} className={`relative inline-flex h-5 w-9 items-center rounded-full ${c.is_active ? 'bg-purple-600' : 'bg-gray-200'}`} title={c.is_active ? 'Disable' : 'Enable'}>
                    <span className={`inline-block h-3.5 w-3.5 transform rounded-full bg-white dark:bg-gray-900 shadow transition-transform ${c.is_active ? 'translate-x-4' : 'translate-x-1'}`} />
                  </button>
                  <Link href={`/gateway/channels/${c.id}`} className="p-1.5 rounded text-gray-400 dark:text-gray-500 hover:text-gray-600 dark:hover:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800" title="Open">
                    <Edit2 className="w-4 h-4" />
                  </Link>
                  <button onClick={() => remove(c.id)} className="p-1.5 rounded text-gray-400 dark:text-gray-500 hover:text-red-500 hover:bg-red-50" title="Delete">
