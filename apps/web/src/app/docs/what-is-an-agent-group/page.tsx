...
export const metadata = { title: 'What is an Agent Group — Docs' }

export default function WhatIsAnAgentGroupDoc() {
  return (
    <DocPage
      title="What is an Agent Group?"
      subtitle="Agent groups let you chain multiple agents together so each one handles a specific part of a complex task."
    >
      <p>
        ...
        expertise and different tool access. <strong>Agent groups</strong> solve this by running
      ...
      <p>
        ...
        additional context. The final output of the last agent becomes the group&apos;s output.
      </p>
      ...
      <p>
        Go to <a href="/agent-groups/new">Agent Groups → New Group</a>. You configure:
      </p>
      <table>
        <thead><tr><th>Field</th><th>Description</th></tr></thead>
        <tbody>
          <tr><td><strong>Name</strong></td><td>Identifies the group in the UI and API</td></tr>
          <tr><td><strong>Description</strong></td><td>Explains what the group does — shown in the docs playground</td></tr>
      ...
      <Callout type="tip">
        Each agent in a group should be configured for its specific role. Give it a tight system prompt,
      ...
      <p>
        When you invoke a group (via the playground or API), the runtime:
      </p>
      <ol>
        <li>Creates a <strong>group run record</strong> with a unique run ID.</li>
        ...
        <li>Returns the final agent&apos;s output as the group&apos;s result.</li>
      </ol>

      <h2>Group Runs vs. Agent Runs</h2>
      <table>
        <thead><tr><th></th><th>Single Agent Run</th><th>Group Run</th></tr></thead>
        <tbody>
          <tr><td><strong>Run ID</strong></td><td>Single run record</td><td>One run record per agent + one group record</td></tr>
          <tr><td><strong>Status</strong></td><td>Tracks one agent</td><td>Tracks across all agents in pipeline</td></tr>
          <tr><td><strong>Invoke endpoint</strong></td><td><code>/invoke/agents/:id</code></td><td><code>/invoke/groups/:id</code></td></tr>
      ...
      <h2>Next Steps</h2>
      <ul>
        <li><a href="/agent-groups/new">Create your first agent group</a></li>
        <li><a href="/docs/invoke-api">Invoke a group via the API</a></li>
        <li><a href="/docs/run-states">Understand group run states</a></li>
      </ul>
    </DocPage>
  )
}
