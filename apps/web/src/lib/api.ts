...
export const mcpAPI = {
  ...
  testTool: (serverId: string, name: string, input: unknown) =>
    api.post<{ output: unknown; error: string; latency_ms: number; request: string; response: string }>(
...
export const gatewayAPI = {
  ...
  syncLIDs: (id: string) => api.post(`/gateway/channels/${id}/adapter/sync-lids`),
  syncContacts: (id: string) => api.post(`/gateway/channels/${id}/adapter/sync-contacts`),
