    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 my-5 mb-8">
      {items.map((c) => (
        <a
          key={c.href}
          href={c.href}
          className="flex gap-4 items-start p-4 rounded-xl border border-border-strong bg-surface no-underline transition-[border-color,box-shadow] duration-150 hover:border-violet-600 hover:shadow-[0_0_0_3px_rgba(83,74,183,0.08)]"
        >
          {c.badge && (
