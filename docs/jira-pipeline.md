...
2. **Connect credentials in Settings → Claude Code** (per workspace, encrypted,
   ...
   Settings → Claude Code is credentials only — repositories, the readiness
   checklist, and recent session activity live on the **Claude Code** page
   (Build section of the sidebar).
3. **Onboard repos** — two paths, one rule: *indexed ≠ writeable*.
   - Sync a **GitHub connector**: every synced repo is adopted into the
     ...
     *enable sessions* toggle (Claude Code → Repositories tab) for each repo
     the pipeline may modify.
   - Or **Add repository** on the same tab: explicit onboarding clones and
   ...
   The readiness strip at the top of the Claude Code page shows the runner's
   executor mode — **stub mode is simulation only** (no code written, fake
   branches); set `RUNNER_EXECUTOR=claude` for real sessions.
